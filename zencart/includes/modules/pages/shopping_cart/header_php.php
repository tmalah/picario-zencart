...
for ($i=0, $n=sizeof($products); $i<$n; $i++) { //echo '<pre>'; print_r($products); echo '</pre>'; exit();
  if (($i/2) == floor($i/2)) {
    $rowClass="rowEven";
  } else {
    $rowClass="rowOdd";
  ...
  if (isset($products[$i]['attributes']) && is_array($products[$i]['attributes'])) {
    ...
    foreach ($products[$i]['attributes'] as $option => $value) {
      ...
      if ($option == 25 && $value == 28) {
        if (isset($_SESSION['picario'][$products[$i]['id']]['coat_price']) && $_SESSION['picario'][$products[$i]['id']]['coat_price'] != '') {
            $attrArray[$option]['options_values_price'] = (float)$_SESSION['picario'][$products[$i]['id']]['coat_price'];
        }
      }
      
      if ($option == 62 && isset($_SESSION['picario'][$products[$i]['id']]['fabric_attr_price']) && $_SESSION['picario'][$products[$i]['id']]['fabric_attr_price'] > 0) {
        $attrArray[$option]['options_values_price'] = (float)$_SESSION['picario'][$products[$i]['id']]['fabric_attr_price'];
      }
  ...
  //$productsImage = isset($_SESSION['image_link']) && $_SESSION['image_link'] != '' ? '<img src="'.$_SESSION['image_link'].'" />' : (IMAGE_SHOPPING_CART_STATUS == 1 ? zen_image(DIR_WS_IMAGES . $products[$i]['image'], $products[$i]['name'], IMAGE_SHOPPING_CART_WIDTH, IMAGE_SHOPPING_CART_HEIGHT) : '');
  //echo '<pre>'; print_r($products); echo '</pre>'; exit();
  if (isset($_SESSION['picario'][$products[$i]['id']]['image_link']) && $_SESSION['picario'][$products[$i]['id']]['image_link'] !='') {
    $productsImage = '<img src="'.$_SESSION['picario'][$products[$i]['id']]['image_link'].'" />';
  } else {
