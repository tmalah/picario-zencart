...
class shoppingCart extends base {
   ...
   */
  function restore_contents() { //echo 'restore'; exit();
    global $db;
    ...
    if (is_array($this->contents)) {
      ...
      while (list($products_id, ) = each($this->contents)) {
        ...
        if (isset($_SESSION['picario'][$products_id]['image_link'])) {
            $image_link = $_SESSION['picario'][$products_id]['image_link'];
        } else {
            $image_link = '';
        }
        ...
        if ($product->RecordCount()<=0) {
          ...
          $sql = "insert into " . TABLE_CUSTOMERS_BASKET . "
                                (customers_id, products_id, customers_basket_quantity,
                                 customers_basket_date_added, picario_image_link)
                                 values ('" . (int)$_SESSION['customer_id'] . "', '" . zen_db_input($products_id) . "', '" .
          $qty . "', '" . date('Ymd') . "', '".$image_link."')";
    ...
    $products_query = "select products_id, customers_basket_quantity, picario_image_link
                         from " . TABLE_CUSTOMERS_BASKET . "
                         where customers_id = '" . (int)$_SESSION['customer_id'] . "'";
    ...
    while (!$products->EOF) {
      ...
      if ($products->fields['picario_image_link'] != '') {
        $_SESSION['picario'][$products->fields['products_id']]['image_link'] = $products->fields['picario_image_link'];
      }
	  ...
	  $this->contents[$products->fields['products_id']]['attributes']['attributes_updated'] = 1;
      $products->MoveNext();
    }
  ...
  function add_cart($products_id, $qty = '1', $attributes = '', $notify = true) {
	...
	if (!$this->in_cart($products_id)) {
		$products_id = zen_get_uprid_width_random($products_id, $attributes);
	}
    if ($notify == true) {
    ...
    if ($this->in_cart($products_id)) {
      ...
      if (is_array($attributes)) {
		...
		$this->contents[$products_id]['attributes']['attributes_updated'] = 1;
      }
    }
    $this->cleanup();
   ...
   * @global object access to the db object
   */
  function update_quantity($products_id, $quantity = '', $attributes = '') { //echo 'update qty'; exit();
    global $db;
  ...
  function calculate() {
    ...
    while (list($products_id, ) = each($this->contents)) {
      ...
      if ($product = $db->Execute($product_query)) {
		...
		$calcualte_measurement = $this->calculate_measurement_price($this->contents[$products_id]['measurement_profile_id'], $product->fields['products_id'], $qty);
		$products_price += $calcualte_measurement['price'];
      ...
      if (isset($this->contents[$products_id]['attributes'])) {
        ...
        while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
          ...
          */
          ...
          if ($option == 25 && $value == 28) {
            if (isset($_SESSION['picario'][$products_id]['coat_price']) && $_SESSION['picario'][$products_id]['coat_price'] != '') {
                $attribute_price->fields['options_values_price'] = (float)$_SESSION['picario'][$products_id]['coat_price'];
            }
          }
          //echo $option.' : '.$_SESSION['picario'][$products_id]['fabric_attr_price']; exit();
          if ($option == 62 && isset($_SESSION['picario'][$products_id]['fabric_attr_price']) && $_SESSION['picario'][$products_id]['fabric_attr_price'] > 0) {
            $attribute_price->fields['options_values_price'] = (float)$_SESSION['picario'][$products_id]['fabric_attr_price'];
          }
...
// validate cart contents for checkout
		...
		$calcualte_measurement = $this->calculate_measurement_price($this->contents[$products_id]['measurement_profile_id'], $products_id, $new_qty);
        
        if ((int)$products_id == 2624 && isset($this->contents[$products_id]['attributes'][62])) { 
            //  get fabric name
            $sql = "SELECT products_options_values_name FROM ".TABLE_PRODUCTS_OPTIONS_VALUES."
                    WHERE products_options_values_id = ".$this->contents[$products_id]['attributes'][62];
            $fabric_info = $db->execute($sql);
            $productsName = $fabric_info->fields['products_options_values_name'].' Suit';
        } else { 
            $productsName = $products->fields['products_name'];
        }

        //  get picario images from cart table
        $picario_sql = "SELECT picario_image_link FROM ".TABLE_CUSTOMERS_BASKET."
                        WHERE customers_id = '" . (int)$_SESSION['customer_id'] . "'
                        AND products_id = '" . zen_db_input($products_id) . "'";
        $picario_res = $db->execute($picario_sql);
        if ($picario_res->RecordCount() > 0 && $picario_res->fields['picario_image_link'] != '') {
            $_SESSION['picario'][$products_id]['image_link'] = $picario_res->fields['picario_image_link'];
        }
        ...
        $products_array[] = array('id' => $products_id,
                                  ...
                                  'price' => ($products->fields['product_is_free'] =='1' ? 0 : $products_price+$calcualte_measurement['price']),
        //                                    'quantity' => $this->contents[$products_id]['qty'],
                                  'quantity' => $new_qty,
                                  'weight' => $products->fields['products_weight'] + $this->attributes_weight($products_id),
                                  // fix here
                                  'final_price' => ($products_price + $this->attributes_price($products_id) + $calcualte_measurement['price']),
								  ...
								  'measurement_price' => $calcualte_measurement['price'],
								  'measurement_arributes' => (isset($calcualte_measurement['attributes']) ? $calcualte_measurement['attributes'] : ''),
								  ...
								  'products_measurement_type' => $products->fields['products_measurement_type']);
      }
    }
  ...
  function actionUpdateAttributes($goto, $parameters) {
	global $messageStack, $db;

	if (isset($_POST['products_id'])) {
	  $gen_product_id = $_POST['gen_products_id'];
      
       if (isset($_POST['image_link'])) $_SESSION['picario'][$gen_product_id]['image_link'] = $_POST['image_link']; 
       if (isset($_POST['coat_price'])) $_SESSION['picario'][$gen_product_id]['coat_price'] = $_POST['coat_price']; 
       if (isset($_POST['fabric_attr_price'])) $_SESSION['picario'][$gen_product_id]['fabric_attr_price'] = $_POST['fabric_attr_price']; 
...
//          $messageStack->add('header', 'REMOVE ME IN SHOPPING CART CLASS BEFORE RELEASE<br/><BR />' . ERROR_CORRECTIONS_HEADING . $the_list, 'error');
			...
			if ($this->in_cart($gen_product_id)) {
				$products_id = $gen_product_id;
				...
				unset($this->contents[$products_id]['attributes']);
				...
				while (list($option, $value) = each($attributes)) {
				  ...
				  if (!$blank_value) {
					if (is_array($value) ) {
					  reset($value);
					  while (list($opt, $val) = each($value)) {
						if($val == 0) unset($this->contents[$products_id]['attributes'][$option.'_chk'.$opt]);
						else
						$this->contents[$products_id]['attributes'][$option.'_chk'.$val] = $val;
					  }
					} else {
					...
					if (isset($_SESSION['customer_id'])) {
					  ...
					  if (is_array($value) ) {
						reset($value);
						while (list($opt, $val) = each($value)) {
						  $val_sql = "select customers_basket_attributes_id from " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES ."
									where customers_id=".(int)$_SESSION['customer_id']." and products_id='".zen_db_input($products_id)."' and products_options_id='".(int)$option.'_chk'. (int)$val."'";
						  $has_attr = $db->Execute($val_sql);
						  if ($has_attr->RecordCount()>0) {
							$sql = "update " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " set products_options_value_id=".(int)$val . " where customers_basket_attributes_id=".$has_attr->fields['customers_basket_attributes_id'];
							$db->Execute($sql);
						  }else
						  {
							  $products_options_sort_order= zen_get_attributes_options_sort_order(zen_get_prid($products_id), $option, $opt);
						...
						$val_sql = "select customers_basket_attributes_id from " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES ."
									where customers_id=".(int)$_SESSION['customer_id']." and products_id='".zen_db_input($products_id)."' and products_options_id=".(int)$option;
						$has_attr = $db->Execute($val_sql);
						if ($has_attr->RecordCount()>0) {
							$sql = "update " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " set products_options_value_id=".(int)$value . ", products_options_value_text='".$attr_value."' where customers_basket_attributes_id=".$has_attr->fields['customers_basket_attributes_id'];
							$db->Execute($sql);
						}else
						{
							$products_options_sort_order= zen_get_attributes_options_sort_order(zen_get_prid($products_id), $option, $value);
...
// display message if all is good and not on shopping_cart page
	  ...
	  $para = zen_get_all_get_params($parameters);
	  if($goto == FILENAME_MEASUREMENTS) $para .= '&products_id='.$gen_product_id;

      zen_redirect(zen_href_link($goto, $para));
    }
  }
  
  function saveCustomerAttributes($goto, $parameters) {
	global $messageStack, $db;
	$attributes = isset($_POST['id']) ? $_POST['id'] : "";
	...
	if (is_array($attributes)) {
		...
		while (list($option, $value) = each($attributes)) {
			...
			if(!$blank_value) {
				$attr_value = zen_db_input($attr_value);
				$csql = "select id from " . TABLE_CUSTOMERS_MEASUREMENTS . ' where customers_id = ' . $customers_id . ' and products_options_id = ' . (int)$option;
					
				$mto = $db->Execute($csql);
				if ($mto->RecordCount() > 0) {
					$sql_data_array = array('measurement_value' => $attr_value, 
											'last_modified' => 'now()'
											);
					zen_db_perform(TABLE_CUSTOMERS_MEASUREMENTS, $sql_data_array, 'update', 'id='.$mto->fields['id']);
				} else {
					$sql_data_array = array('customers_id' => $customers_id,
											'products_options_id' => (int)$option,
											'measurement_value' => $attr_value, 
											'date_added' => 'now()',
											'last_modified' => 'now()'
											);
					zen_db_perform(TABLE_CUSTOMERS_MEASUREMENTS, $sql_data_array);
				} // insert customer measures
...
//          $messageStack->add('header', 'REMOVE ME IN SHOPPING CART CLASS BEFORE RELEASE<br/><BR />' . ERROR_CORRECTIONS_HEADING . $the_list, 'error');
          ...
          $gen_product_id = $this->add_cart($_POST['products_id'], $this->get_quantity(zen_get_uprid($_POST['products_id'], $real_ids))+($new_qty), $real_ids);
...
// display message if all is good and not on shopping_cart page
	  ...
	  $para = zen_get_all_get_params($parameters);
	  if($goto == FILENAME_CUSTOMIZATIONS) $para .= '&products_id='.$gen_product_id;
...
//            $messageStack->add_session('shopping_cart', 'A: ' . ERROR_QUANTITY_ADJUSTED . zen_get_products_name($products) . ' - ' . $old_quantity . ' => ' . $new_qty, 'caution');
  ...
  function calculate_measurement_price($measurement_id, $products_id, $qty=1) {
	 global $db;
	$measurement_option_price = 0;
	$measurement_arributes = array();
	if(!empty($_SESSION['customer_id'])) {
		$dynamic_options_array = array();
		$dynamic_options_query = $db->Execute("select dop.name, dop.id, dop.price, dop.rule, dop.description from " . TABLE_DYNAMIC_OPTIONS_PRICE_PRODUCTS . " dopp join " . TABLE_DYNAMIC_OPTIONS_PRICE . " dop on dopp.dynamic_price_rule_id = dop.id where dopp.products_id = " . (int)$products_id); 
		while (!$dynamic_options_query->EOF) {
			$dynamic_options_array[$dynamic_options_query->fields['id']] = array('name' => $dynamic_options_query->fields['name'],
																		 'price' => $dynamic_options_query->fields['price'],
																		 'rule' => $dynamic_options_query->fields['rule'],
																		 'description' => $dynamic_options_query->fields['description']);
			$dynamic_options_query->MoveNext();
		}
		if(count($dynamic_options_array) > 0) {
			$customers_measurement_option_name_array = array();
			$customers_measurement_option_value_array = array();
			$customers_measurement_option_query = $db->Execute("select cm.measurement_value, po.products_options_name from " . TABLE_CUSTOMERS_MEASUREMENTS . " cm join " . TABLE_PRODUCTS_OPTIONS . " po on cm.products_options_id = po.products_options_id and po.language_id = " . $_SESSION['languages_id'] . " where cm.customers_id = " . (int)$_SESSION['customer_id']);
			while (!$customers_measurement_option_query->EOF) {
				$customers_measurement_option_name_array[] = '('. $customers_measurement_option_query->fields['products_options_name'] . ')';
				$customers_measurement_option_value_array[] = $customers_measurement_option_query->fields['measurement_value'];
				$customers_measurement_option_query->MoveNext();
			}
			foreach($dynamic_options_array as $key=>$dynamic_option) {
				$newrule = str_replace($customers_measurement_option_name_array, $customers_measurement_option_value_array, $dynamic_option['rule']);
				$pos = strpos($newrule, '(');
				$value_field = false;
				if ($pos === false) {
				eval('$value_field = ' . $newrule . ';');
				}
				if($value_field) {
					$measurement_option_price +=$dynamic_option['price'];
					$measurement_arributes[] = '+$'.$dynamic_option['price']*$qty .' (' . $dynamic_option['description'] . ')';
				}
			}
		}
	}
	return array('price'=>$measurement_option_price, 'attributes'=>$measurement_arributes);
  }
}
