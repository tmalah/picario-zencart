...
class order extends base {
  ...
  function query($order_id) {
    ...
    $orders_products_query = "select orders_products_id, products_id, products_name,
                                 ...
                                 products_discount_type_from, products_measurement_type
    ...
    while (!$orders_products->EOF) {
      ...
      $this->products[$index] = array('qty' => $new_qty,
									  ...
									  'products_measurement_type' => $orders_products->fields['products_measurement_type']);
...
*/
	...
	#<!-- BOF - Order Status by Product Id hack by Minhmt@gmail.com -->
	 ...
	 $products_order_status_count	=	0; 
#<!-- EOF - Order Status by Product Id hack by Minhmt@gmail.com -->	
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
		...
		#<!-- BOF - Order Status by Product Id hack by Minhmt@gmail.com -->
			// check if the product in products_order_status list
			for ($j=0, $k=sizeof($products_order_status_array); $j<$k;  $j++)  {
				if  ((int)$products[$i]['id']==$products_order_status_array[$j]['products_id']) {
					$new_default_order_status_id=$products_order_status_array[$j]['order_status_id'];	
					$products_order_status_count++; // count the number of products in the list 
				}
		...
		$sql = "select products_measurement_type from " . TABLE_PRODUCTS . " where products_id = " . $product_id;
		$product = $db->Execute($sql);
		$measurement_type = $product->fields['products_measurement_type'];
		if($measurement_type != '' && $measurement_type != 0) {
			$use_measurement_sql = "select use_measurement from " . TABLE_PRODUCT_MEASUREMENT_TYPES . " where type_id = " . $measurement_type;
			$use_measurement = $db->Execute($use_measurement_sql);
			$use_measurement_count = $use_measurement->recordCount();
			if($use_measurement_count > 0 && $use_measurement->fields['use_measurement'] != '') {
				$sql = "select po.products_options_type, po.products_options_measurement_value as default_value, cm.measurement_value, po.products_options_id, po.products_options_measurement_unit, po.products_options_name, pmt.type_name, pmt.type_special
					from " . TABLE_PRODUCT_MEASUREMENT_TYPES_TO_OPTIONS . " pmtto join " . TABLE_PRODUCT_MEASUREMENT_TYPES . " pmt
					on pmtto.product_measurement_type_id = pmt.type_id join " .	TABLE_PRODUCTS_OPTIONS . " po
					on po.products_options_id = pmtto.product_options_id 
					left join " . TABLE_CUSTOMERS_MEASUREMENTS . " cm 
					on po.products_options_id = cm.products_options_id 
					where (pmtto.product_measurement_type_id in (" . $use_measurement->fields['use_measurement'] . ") or pmt.type_special = 1) and cm.customers_id = " . (int)$_SESSION['customer_id'];
				...
				$sql = "select po.products_options_type, po.products_options_measurement_value as default_value, cm.measurement_value, po.products_options_id, po.products_options_measurement_unit, po.products_options_name, pmt.type_name, pmt.type_special
					...
					left join " . TABLE_CUSTOMERS_MEASUREMENTS . " cm 
					on po.products_options_id = cm.products_options_id 
					where (pmtto.product_measurement_type_id = " . $measurement_type . " or pmt.type_special = 1) and cm.customers_id = " . (int)$_SESSION['customer_id'];
			}
			$measures = $db->Execute($sql);
			while (!$measures->EOF) {
				$opt_id = $measures->fields['products_options_id'];
				if($use_measurement_count > 0 && $use_measurement->fields['use_measurement'] != '' && !$measures->fields['type_special']) {
					//$opt_id = $measures->fields['type_name'] . '_' . $measures->fields['products_options_id'];
				}
				$opt_id = $measures->fields['products_options_id'];
				if($measures->fields['products_options_type'] == PRODUCTS_OPTIONS_TYPE_TEXT) {
					$value = ($measures->fields['measurement_value'] == 0) ? $measures->fields['default_value'] : $measures->fields['measurement_value'];
					if(strtolower($measures->fields['products_options_name']) == 'height') {
						$v1 = floor($value / 12);
						$v2 = $value - $v1*12;
						$value = $v1 . ' feet ' . $v2 . ' inches';
					}
					if(!empty($measures->fields['products_options_measurement_unit']))
						$value .= '';
						//$value .= ' ' . $measures->fields['products_options_measurement_unit'];
					if(empty($value)) $value = 0;
					$products[$i]['attributes_values'][$opt_id] = $value;
					$products[$i]['attributes'][$opt_id] = PRODUCTS_OPTIONS_VALUES_TEXT_ID;
				}else {
					$products[$i]['attributes'][$opt_id] = $measures->fields['measurement_value'];
				}
				$measures->MoveNext();
			}
	  }
      ...
      $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                      ...
                                      'id' => $products[$i]['id'],
									  'products_measurement_type' => $products[$i]['products_measurement_type'],
									  'measurement_price' => $products[$i]['measurement_price'],
									  'measurement_arributes' => $products[$i]['measurement_arributes'],
                                      'rowClass' => $rowClass);
      ...
      if ($products[$i]['attributes']) {
        ...
        while (list($option, $value) = each($products[$i]['attributes'])) {
          ...
          */
          ...
          $this->products[$index]['attributes'][$subindex] = array('option' => $measure_name . $attributes->fields['products_options_name'],
                                                                   'value' => $attr_value,
...
*/
	...
	#<!-- BOF - Order Status by Product Id hack by Minhmt@gmail.com -->
		if  ($products_order_status_count==1) {
			$this->info['order_status']	=	$new_default_order_status_id;
		} 
#<!-- EOF - Order Status by Product Id hack by Minhmt@gmail.com -->
  ...
  function  create_add_products($zf_insert_id, $zf_mode = false) {
    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
      ...
      $sql_data_array = array('orders_id' => $zf_insert_id,
							  ...
							  'products_measurement_type' => $this->products[$i]['products_measurement_type']);
      zen_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array);

      $order_products_id = $db->Insert_ID();

	  if($this->products[$i]['products_measurement_type'] != 0 && $this->products[$i]['products_measurement_type'] != 4) {
		$db->Execute("update " . TABLE_CUSTOMERS . ' set customers_lock_measurements = 1 where customers_id=' . $_SESSION['customer_id']);
	  }
      ...
      if (isset($this->products[$i]['attributes'])) {
		...
		$measurements = $db->Execute("select products_options_id from " . TABLE_PRODUCTS_OPTIONS . " where products_options_measurement = 1");
		while (!$measurements->EOF) {
			$measurements_array[$measurements->fields['products_options_id']] = $measurements->fields['products_options_id'];
			$measurements->MoveNext();
		}
        for ($j=0, $n2=sizeof($this->products[$i]['attributes']); $j<$n2; $j++) {
		
		  $option = $this->products[$i]['attributes'][$j]['option_id'];
		  $pos = strpos($option, '___');
		  $measure_name = '';
		  if ($pos !== false) {
			$measure_name = substr($option, 0, $pos) . ' - ';
          ...
          if (DOWNLOAD_ENABLED == 'true') {
            ...
            $attributes_values = $db->Execute("select popt.products_options_name, poval.products_options_values_name,
                                 ...
                                 where pa.products_id = '" . zen_db_input($this->products[$i]['id']) . "' and pa.options_id = '" . (int)$option . "' and pa.options_id = popt.products_options_id and pa.options_values_id = '" . (int)$this->products[$i]['attributes'][$j]['value_id'] . "' and pa.options_values_id = poval.products_options_values_id and popt.language_id = '" . $_SESSION['languages_id'] . "' and poval.language_id = '" . $_SESSION['languages_id'] . "'");
			if($attributes_values->recordCount() == 0) {
				$attributes_values = $db->Execute("select products_options_name
                                 from " . TABLE_PRODUCTS_OPTIONS . "
                                 where products_options_id = '" . (int)$option . "' and language_id = '" . $_SESSION['languages_id'] . "'");
			}
          }

        if ($option == 25 && $this->products[$i]['attributes'][$j]['value'] == 28) {
            //if (isset($_SESSION['coat_price']) && $_SESSION['coat_price'] != '') {
            if (isset($_SESSION['picario'][$this->products[$i]['id']]['coat_price']) && $_SESSION['picario'][$this->products[$i]['id']]['coat_price'] > 0) {
                $attributes_values->fields['options_values_price'] = (float)$_SESSION['picario'][$this->products[$i]['id']]['coat_price'];
            }
        }
        //echo '<pre>'; print_r($this->products[$i]); echo '</pre>'; exit();
        if ($option == 62 && isset($_SESSION['picario'][$this->products[$i]['id']]['fabric_attr_price']) && $_SESSION['picario'][$this->products[$i]['id']]['fabric_attr_price'] > 0) {
            $attributes_values->fields['options_values_price'] = (float)$_SESSION['picario'][$this->products[$i]['id']]['fabric_attr_price'];
        }

          //clr 030714 update insert query.  changing to use values form $order->products for products_options_values.
          $sql_data_array = array('orders_id' => $zf_insert_id,
                                  'orders_products_id' => $order_products_id,
                                  'products_options' => $measure_name . $attributes_values->fields['products_options_name'],
		  ...
		  if(!array_key_exists($option, $measurements_array)) {
			$this->products_ordered_attributes .= "\n\t" . $attributes_values->fields['products_options_name'] . ' - ' . zen_decode_specialchars($this->products[$i]['attributes'][$j]['value']);
		  }
		}
	  }
	  if(isset($this->products[$i]['measurement_arributes']) && is_array($this->products[$i]['measurement_arributes'])) {
		foreach($this->products[$i]['measurement_arributes'] as $mkey => $mvalue) {
			$sql_data_array = array('orders_id' => $zf_insert_id,
                              'orders_products_id' => $order_products_id,
                              'products_options' => '',

                                         'products_options_values' => $attributes_values->fields['products_options_values_name'],
                              'products_options_values' => $mvalue,
                              ...
                              'attributes_qty_prices' => 0,
                              'attributes_qty_prices_onetime' => 0,
...
/* END: ADD MY CUSTOM DETAILS */
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
    ...
    EMAIL_SEPARATOR . "\n";
	if($this->products_ordered == '') {
		for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
			$email_order .=  $this->products[$i]['qty'] . ' x ' . $this->products[$i]['name'] . ($this->products[$i]['model'] != '' ? ' (' . $this->products[$i]['model'] . ') ' : '') . ' = ' .
			  $currencies->display_price($this->products[$i]['final_price'], $this->products[$i]['tax'], $this->products[$i]['qty']) .
			  ($this->products[$i]['onetime_charges'] !=0 ? "\n" . TEXT_ONETIME_CHARGES_EMAIL . $currencies->display_price($this->products[$i]['onetime_charges'], $this->products[$i]['tax'], 1) : '') .
			  $this->products_ordered_attributes . "\n";
		}
	} else {
		$email_order .= $this->products_ordered;
	}
	$email_order .= EMAIL_SEPARATOR . "\n";
    $html_msg['PRODUCTS_TITLE'] = EMAIL_TEXT_PRODUCTS;
	if($this->products_ordered_html == '') {
		for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
			$this->products_ordered_html .=
			  '<tr>' . "\n" .
			  '<td class="product-details" align="right" valign="top" width="30">' . $this->products[$i]['qty'] . '&nbsp;x</td>' . "\n" .
	...
	if(isset($_SESSION['sendto']) && $_SESSION['sendto'] != '')
		...
		$address = array(
			...
			'street_address' => $this->delivery['street_address'],
			'suburb' => $this->delivery['suburb'],
		...
		$html_msg['ADDRESS_DELIVERY_DETAIL']    = ($this->content_type != 'virtual') ? zen_address_format($this->delivery['format_id'], $address, true, '', "<br />") : 'n/a';
	}
    ...
    if ($this->content_type != 'virtual') {
      ...
      EMAIL_SEPARATOR . "\n";
	  if(isset($_SESSION['sendto']) && $_SESSION['sendto'] != '')
		$email_order .= zen_address_label($_SESSION['customer_id'], $_SESSION['sendto'], 0, '', "\n") . "\n";
	  else {
		$email_order .= ($this->content_type != 'virtual') ? zen_address_format($this->delivery['format_id'], $address, true, '', "<br />") : 'n/a';
	  }
    }
	...
	if(isset($_SESSION['billto']) && $_SESSION['billto'] != '')
		$email_order .= zen_address_label($_SESSION['customer_id'], $_SESSION['billto'], 0, '', "\n") . "\n\n";
	else {
		$address = array(
			'name' => $this->billing['name'],
			'street_address' => $this->billing['street_address'],
			'suburb' => $this->billing['suburb'],
		...
		$email_order .= zen_address_label($this->billing['format_id'], $address, 0, '', "\n") . "\n\n";
	}
	...
	if(isset($_SESSION['billto']) && $_SESSION['billto'] != '')
		...
		$html_msg['ADDRESS_BILLING_DETAIL'] = ($this->content_type != 'virtual') ? zen_address_format($this->billing['format_id'], $address, true, '', "<br />") : 'n/a';
	}
	...
	if($this->customer['firstname'] == '')
		zen_mail($this->customer['name'], $this->customer['email_address'], EMAIL_TEXT_SUBJECT . EMAIL_ORDER_NUMBER_SUBJECT . $zf_insert_id, $email_order, STORE_NAME, EMAIL_FROM, $html_msg, 'checkout', $this->attachArray);
	else
